...
Deepkit 프레임워크는 `@d7/app`의 [Deepkit App](./app.md)을 기반으로 하며, 애플리케이션에 가져와 사용할 수 있는 `@d7/framework`의 `FrameworkModule` 모듈을 제공합니다.
...
`FrameworkModule`이 다른 많은 것들(Logger, ApplicationServer, 그리고 [더 많은 것들](https://github.com/marcj/d7/blob/master/packages/framework/src/module.ts))과 마찬가지로 이를 서비스 프로바이더로 등록하므로 `EventDispatcher`를 가져올 수 있습니다.
