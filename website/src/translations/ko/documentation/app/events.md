...
`@d7/app`를 사용한다면 app.listen()을 통해 함수형 리스너를 등록할 수도 있습니다.
...
아래는 @d7/event에서 제공하는 저수준 API의 예시입니다. Deepkit App을 사용할 때는 이벤트 리스너가 EventDispatcher를 통해 직접 등록되지 않고 모듈을 통해 등록됩니다. 하지만 원한다면 저수준 API를 그대로 사용할 수도 있습니다.
...
이벤트 시스템은 @d7/app에 포함되어 있습니다. 단독으로 사용하려면 수동으로 설치할 수 있습니다:
