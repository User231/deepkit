...
dependency injection 컨테이너(DI 컨테이너 또는 IoC 컨테이너라고도 함)는 Deepkit에서 `@d7/injector`로 제공되며, Deepkit Framework에서는 App modules를 통해 이미 통합되어 있습니다. 위의 코드는 `@d7/injector` 패키지의 low-level API를 사용하면 다음과 같이 보입니다.
