...
이 작업이 성공적으로 완료되면, 라이브러리를 내부적으로 사용하는 Deepkit framework를 사용하거나 `@d7/app`을 설치할 수 있습니다.

```sh
npm install @d7/http
```

controller API용 `@d7/http`는 TypeScript annotations에 기반하므로 controller API를 사용할 때 `experimentalDecorators` 기능을 활성화해야 합니다.
