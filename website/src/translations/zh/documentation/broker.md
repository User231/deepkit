...
第二个是位于 [@d7/broker-redis](./package/broker-redis) 的 `BrokerRedisAdapter`，它与 Redis 服务器通信。第三个是 `BrokerMemoryAdapter`，用于测试的内存适配器。
...
如果你需要自定义适配器，可以通过实现 `@d7/broker` 中以下接口之一或多个来创建自己的适配器：
