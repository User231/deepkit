...
使用来自 @d7/event 的 `DataEventToken`：
...
如果你使用 `@d7/app`，也可以使用 app.listen() 来注册函数式监听器。
...
下面是来自 @d7/event 的底层 API 示例。使用 Deepkit 应用时，事件监听器不是直接通过 EventDispatcher 注册，而是通过模块注册。但如果你愿意，仍然可以使用底层 API。
...
事件系统已包含在 @d7/app 中。如果你想单独使用，可以手动安装：
