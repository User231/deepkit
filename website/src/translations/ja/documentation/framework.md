...
Deepkit Framework は `@d7/app` の [Deepkit App](./app.md) を基盤としており、`@d7/framework` の `FrameworkModule` モジュールを提供します。これはアプリケーションにインポートできます。
...
`FrameworkModule` が多くのもの（Logger、ApplicationServer、そして[その他多数](https://github.com/marcj/d7/blob/master/packages/framework/src/module.ts)）と同様に `EventDispatcher` をサービスプロバイダーとして登録しているため、`EventDispatcher` を取得できます。
