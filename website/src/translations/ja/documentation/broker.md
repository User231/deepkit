...
2 つ目は [@d7/broker-redis](./package/broker-redis) にある `BrokerRedisAdapter` で、Redis サーバーと通信します。3 つ目はテスト目的のインメモリアダプターである `BrokerMemoryAdapter` です。
...
カスタムアダプターが必要な場合は、`@d7/broker` にある次のインターフェースのうち 1 つ以上を実装して独自のアダプターを作成できます:
