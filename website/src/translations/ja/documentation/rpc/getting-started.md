...
Deepkit RPC を使用するには、ランタイム型に基づいているため `@d7/type` を正しくインストールしておく必要があります。 [ランタイム型のインストール](../runtime-types.md) を参照してください。

これが完了したら、ライブラリを内部で既に使用している `@d7/rpc` または Deepkit Framework をインストールできます。

```sh
npm install @d7/rpc
```

なお、`@d7/rpc` の Controller クラスは TypeScript のデコレーターに基づいており、この機能を使用するには experimentalDecorators を有効にする必要があります。

サーバーとクライアントがそれぞれ独自の package.json を持つ場合は、`@d7/rpc` パッケージを両方にインストールする必要があります。

サーバーと TCP で通信するには、クライアントとサーバーの両方に `@d7/rpc-tcp` パッケージをインストールする必要があります。
...
以下は、WebSocket と @d7/rpc の低レベル API に基づく完全に動作するサンプルです。Deepkit Framework を使用する場合、Controller はアプリのモジュール経由で提供され、RpcKernel を手動でインスタンス化する必要はありません。
