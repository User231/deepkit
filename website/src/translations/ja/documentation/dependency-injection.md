...
依存性注入コンテナ（DI コンテナまたは IoC コンテナとも呼ばれる）は、Deepkit では `@d7/injector` として提供されており、Deepkit Framework では App モジュールを通じてすでに統合されています。上記のコードは、`@d7/injector` パッケージの低レベル API を使用すると次のようになります。
