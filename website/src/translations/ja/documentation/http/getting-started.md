...
これが完了していれば、`@d7/app` をインストールするか、すでにこのライブラリを内部で使用している Deepkit フレームワークを利用できます。

```sh
npm install @d7/http
```

controller API 用の `@d7/http` は TypeScript のアノテーションに基づいているため、controller API を使用する場合は `experimentalDecorators` を有効にする必要があります。
