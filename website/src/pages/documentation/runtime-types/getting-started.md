...
Deepkit supports standard (TC39) decorators — no special compiler flag is needed to use decorators with TypeScript 5+. The legacy `"experimentalDecorators": true` mode remains fully supported for projects that still need it (e.g. alongside libraries that require it).
