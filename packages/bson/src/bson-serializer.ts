...
function handleObjectLiteral(
    ...
    for (const member of resolveTypeMembers(type)) {
        ...
        const setUndefined = isOptional(member)
            ...
            : isNullable(member)
              ? executeTemplates(propertyState.fork(), { kind: ReflectionKind.null })
              : '';
...
function sizerNumber(type: Type, state: TemplateState) {
    ...
    //NaN is serialized as 0 (double, 8 bytes)
    sizerPropertyNameAware(
        type,
        state,
        `typeof ${state.accessor} === 'bigint' || typeof ${state.accessor} === 'number'`,
        `
        if (Number.isNaN(${state.accessor})) {
            state.size += 8; // NaN serialized as 0 (double)
        } else {
...
function serializeNumber(type: Type, state: TemplateState) {
    state.addCode(`
        if ('bigint' === typeof ${state.accessor}) {
            ...
            if (Number.isNaN(${state.accessor})) {
                //NaN is serialized as 0 (double)
                state.writer.writeType(${BSONType.NUMBER});
                state.writer.writeDouble(0);
            } else if (Math.floor(${state.accessor}) === ${state.accessor} && ${state.accessor} >= ${BSON_INT32_MIN} && ${state.accessor} <= ${BSON_INT32_MAX}) {
                //32bit int
...
function sizerBigInt(type: TypeBigInt, state: TemplateState) {
    ...
    if (binaryBigInt !== undefined) {
        ...
        //per default bigint will be serialized as long, to be compatible with default mongo driver and mongo database.
        //We should add a new annotation, maybe like `bigint & Binary` to make it binary (unlimited size)
        //NaN is converted to 0n
        sizerPropertyNameAware(
            type,
            state,
            `typeof ${state.accessor} === 'bigint' || typeof ${state.accessor} === 'number'`,
            `
            state.size += ${bigIntSize}(Number.isNaN(${state.accessor}) ? 0n : ${state.accessor});
        `,
        );
    } else {
...
function serializeBigInt(type: TypeBigInt, state: TemplateState) {
    ...
    if (binaryBigInt !== undefined) {
        ...
        if ('bigint' === typeof ${state.accessor} || 'number' === typeof ${state.accessor}) {
            //long (NaN is converted to 0n)
            state.writer.writeType(${BSONType.BINARY});
            state.writer.${writeBigInt}(Number.isNaN(${state.accessor}) ? 0n : ${state.accessor});
        }`);
    } else {
