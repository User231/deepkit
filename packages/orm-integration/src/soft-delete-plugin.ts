...
export const softDeletePluginTests = {
    async query(databaseFactory: DatabaseFactory) {
        ...
        // `.lift()` reparameterizes the query's return shape but its `.filter()` still types
        // against the lifted class's `SoftDeleteEntity` base (which has no `id`), so filtering
        // by a concrete column after a lift needs a cast. Filtering before the lift keeps the
        // entity type (see the restore case below).
        await database
            .query(s)
