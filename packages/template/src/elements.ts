...
// `declare global` in a COMPILED module, not an ambient `elements.d.ts`.
// tsc never copies hand-written .d.ts into dist/, and the triple-slash
// reference did not survive declaration emit — so consumers only ever saw this
// namespace when node10 resolution happened to pull the source in. Under
// nodenext (exports-map enforcement) that stopped, and every JSX element in
// example-app failed TS7026. Shipping it as src/elements.ts makes it reach
// consumers through the normal import graph, resolution-mode independent.
...
declare global {
    namespace JSX {
        ...
        type ExtractProps<T> = {
            [P in Extract<NoFunctions<T>, WritableKeysOf<T>>]?: T[P] extends string | number | Date | boolean
                ? T[P]
                : any;
